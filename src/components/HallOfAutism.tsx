            ...
            {!showRequestForm && !showTakedownForm ? (
              ...
              <>
                ...
                <form onSubmit={handleSubmitTakedown} className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setShowTakedownForm(false)} className="flex-1 py-3 rounded-xl bg-white/5 text-white font-bold uppercase text-sm">Cancel</button>
                    <button type="submit" disabled={isUploading} className="flex-1 py-3 rounded-xl bg-red-500 text-white font-bold uppercase text-sm">Submit Takedown</button>
                  </div>
                </form>
              </>
            ) : (
              <>
                ...
                <form onSubmit={handleSubmitRequest} className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter name..." className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent" required />
                  </div>
                  <div>
                    <label className="block text-sm font-bold uppercase tracking-widest text-text-secondary mb-2">Image URL</label>
                    <input type="text" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} placeholder="https://example.com/image.jpg" className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent" required />
                  </div>
                  <div>
                    <label className="block text-sm font-bold uppercase tracking-widest text-text-secondary mb-2">Description</label>
                    <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Why should they be in the Hall of Cornballs?" className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent min-h-[100px] resize-y" />
                  </div>
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setShowRequestForm(false)} className="flex-1 py-3 rounded-xl bg-white/5 text-white font-bold uppercase text-sm">Cancel</button>
                    <button type="submit" disabled={isUploading} className="flex-1 py-3 rounded-xl bg-accent text-white font-bold uppercase text-sm">Submit Request</button>
                  </div>
                </form>
              </>
            )}
        ...
        {isSuperAdmin && (
          ...
          >
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              ...
              <button type="submit" disabled={isUploading} className="w-full py-3 rounded-xl bg-accent text-white font-bold uppercase text-sm">Add to Hall of Cornballs</button>
            </form>
          </motion.div>
        )}
